...
<Callout kind="info">
  우리가 자주 사용하는 **console.log**의 **console** 객체 역시 Web API를
...
<Callout kind="question">
  **Promise**와 **Microtask Queue**  
  그림에는 표시되지 않았지만 기존 Callback Queue에 더해 Microtask Queue(혹은 Job Queue)라는 다른 큐가 존재합니다.
  서버에서 데이터를 받아올 때 주로 사용되는 Promise 객체의 비동기 처리를 위해 Microtask Queue가 사용됩니다.
  Microtask의 우선순위가 일반 Callback Queue보다 높아, 이벤트 루프가 Microtask의 콜백들을 먼저 Call Stack에 올리는 특징이 있습니다.
</Callout>
...
<Callout kind="info">
  재밌는 점은 **setTimeout(showMessage, 0)** 와 같이 0초 뒤에 콜백을 실행시킨다고 하더라도 결과는 동일하게 나타납니다. 
  
  그 이유는 위를 보시면 아시겠지만, Call Stack이 완전히 비워져야 해당 콜백을 Call Stack에 올리기 때문입니다. 
  즉, **글로벌한 실행 맥락(global execution context)** 까지 콜 스택에서 사라져야 `showMessage()` 함수가 처리되는 것입니다.
</Callout>
...
마지막으로 떠나기 전, 자바스크립트 엔진의 Heap 메모리에 대해 다루지 못해 이와 관련된 **Garbage Collection**을 간단히 소개하고 떠날까 한다.

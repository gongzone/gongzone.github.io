...
[공존의 깃허브: 단진자 운동](https://github.com/gongzone/implementations/tree/main/pendulum)
...
따라서 `<canvas id="canvas"></canvas>` 같이 id 선택자를 부여하여 자바스크립트를 이용하여 컨트롤할 수 있도록 만든다.
...
이 역시 별도의 클래스로 모듈화할 수 있지만, 간략하게 표현하기 위하여 해당 파일을 하나의 모듈로 취급하고 변수로서 선언하였다.
...
`pixelRatio`는  현재 표시 장치의 물리적 픽셀과 CSS 픽셀의 비율을 가지고 있다.
**HiDPI, Retina**와 같은 고밀도 디스플레이는 같은 객체를 그릴 때 더 많은 픽셀을 사용하는데, 
이러한 디스플레이들과 표준 디스플레이 간의 렌더링 차이에 대응할 필요가 있다.
즉, `window.devicePixelRatio` 값을 통해 캔버스 스케일을 조정하지 않으면, 
고밀도 디스플레이에서는 화면 렌더링이 뿌옇게 나타날 수 있다는 것이다.

`getFullWidth`와 `getFullHeight`를 함수로 선언한 것은 리사이즈 시 문제를 고려하기 위해서이다. 
즉, 변수로 선언해두면 현재 시점 `<body>`의 내부 너비와 높이가 변수에 한번 할당되고 끝이지만, 
함수로 선언해두면 **호출한 시점**의 너비와 높이 값을 그때마다 반환 받을 수 있기 때문에 함수로 작성해 보았다.

## 4. App Class 구현

같은 결과물이라도 그 목표에 도달하기에는 여러 방법이 존재할 것이다. 
나는 단진자 운동 애니메이션을 구현하는 데에 있어 크게 **두 가지의 역할**로 모듈을 나누기로 했다.
하나는 Canvas 화면을 조정하여 화면에 시뮬레이터를 렌더링하는 `App Class`이며, 
다른 하나는 단진자 운동 관련 데이터를 계산하고 필요한 객체를 그리는 역할의 `SimplePendulumSimulator Class`이다.
이번 장에서는 `App Class`를 구현해보도록 하겠다.

추가적으로, 
이 글의 목표는 <Link to="/posts/simple-pendulum-theory">앞선 글</Link>에서 언급한 내용을 바탕으로 

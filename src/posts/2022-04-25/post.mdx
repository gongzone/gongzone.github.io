---
title: 'Node.js 패키지 매니저, yarn berry'
...
**_(1) npm (node package manager)_**

🔸 npm은 자바스크립트 런타임인 Node.js를 설치하게 되면 자동적으로 사용할 수 있는 디폴트 패키지 매니저입니다.

🔸 npm은 초기에 그렇다할 경쟁자가 없었기에 개발자들로부터 아래와 같은 문제점을 지적당했으나 빠른 업데이트가 이루어지지 않았습니다.

<Block>
  1️⃣ 전반적인 패키지 설치 프로세스가 굉장히 느리다. <br />
  2️⃣ cache의 이점을 제대로 활용하지 않아 첫번째 설치와 두 번째 설치간의 속도 차이가 거의 없었다. <br />
  3️⃣ lock file이 자동적으로 생성되지 않아 npm shrinkwrap 명령어로 해당 프로젝트의 의존성 버전을
  고정시켜야 했다. <br />
</Block>

🔸 yarn이 출시된 이후 현재 npm은 많은 개선과정을 거쳐 기존 문제점 다수를 해결하였습니다.

**_(2) yarn (yet another resource negotiator) v1 _**

🔹 yarn은 Facebook에서 2016년에 출시한 패키지 매니저입니다.

🔹 yarn은 위에서 언급한 npm의 고질적 문제를 해결한 솔루션이였기에, 많은 개발자들이 npm에서 yarn을 이용하는 것으로 옮겨갔습니다.

🔹 yarn 역시 기존 npm registry(패키지 에코시스템)를 사용할 수 있었기에, npm에서 yarn으로 쉽게 전환할 수 있었습니다.

🔹 yarn version 1(Classic)은 공식적으로 지원이 중지된 상태입니다.

**_(3) 새로운 패키지 매니저: yarn v2+, pnpm_**
...
React를 처음 접할 때 다음과 같은 명령어를 많이 보셨을겁니다. <br /><br />

<strong>npx create-react-app</strong> <br />
<br />
✔ npx(node package execute)는 5.2.0 version 이상의 npm을 설치할 시 자동적으로 설치가 되는 CLI 도구입니다. <br />
<br />✔ npx은 간단히 말해서, <strong> 특정 패키지를 실행시키는 역할을 수행</strong>
합니다. <strong>심지어 node_modules 폴더에 필요한 의존성을 설치하지 않더라도 말이죠.</strong> <br />
<br />
✔따라서 보통의 경우 버전 업데이트가 잦은 모듈을 npm registry에 접근하여 <strong>일회성 명령</strong>
으로 실행시킬 때 주로 사용합니다. <br />
<br />
✔ npm에 npx가 있다면 yarn에는 그와 같은 유사한 도구로 dlx가 존재합니다. <br />
<br />
2️⃣ <strong>nvm</strong> <br />
<br />
✔ Node.js를 여러버전으로 설치해서 관리하게 해주는 도구입니다.
<br />
<br />
...
**_(1) 기존 npm과 yarn v1의 문제점: node_modules_**
...
<strong>유령의존성 👻</strong> <br />
- <strong>package.json에 명시해 놓지 않았으나(내 프로젝트가 직접 의존하지 않으나) 사용할 수 있는 의존성을 말합니다.</strong>
hoisting을 통하여 어떤 의존성 A가 의존하는 의존성 B가 중복되어 트리 구조 상 끌어 올라와졌을 때, 
우리는 그 의존성 B를 직접 사용할 수 있게 됩니다. 그리고 의존성 A를 삭제한 순간 의존성 B도 조용히 사라져 사용할 수 없게 됩니다.<br /><br />
이러한 <strong>통제와 예측이 불가능한 상황을 야기할 수 있기에</strong> 유령 의존성은 문제가 될 수 있습니다.
...
기존 패키지 매니저에서는 node_modules 폴더를 ignore 시키는게 일반적이였습니다. yarn berry의 경우는 아래와 같습니다. <br />

<a href="https://yarnpkg.com/getting-started/qa#which-files-should-be-gitignored">
  Which file should be ignored? - yarn 공식 홈페이지
</a>
...
<a href="https://yarnpkg.com/getting-started/editor-sdks">Editor SDKs - yarn 공식 홈페이지</a>
...
**2) ZipFS라는 VSCode 확장 프로그램을 설치합니다.**

<inlineCode>
  <a href="https://marketplace.visualstudio.com/items?itemName=arcanis.vscode-zipfs">ZipFS 링크</a>
</inlineCode>
